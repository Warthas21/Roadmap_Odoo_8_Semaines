...
def start_pos(inventory, users):
    ...
    if current_user is None:  # Si on a pas trouvé d'utilisateur de la liste
        print("Accès refusé !")
    ...
    ):  # isinstance vérifie le type. -> if current_user is manager = true
    ...
    while True:
        ...
        try:
            id_produit = int(input(
                "Quel produit(ID)? (0 = Quitter, 99=Restock)"
                ))
            if id_produit == 0:
                save_data(inventory)
                save_users(users)
                ...
                if isinstance(current_user, Manager):
                    id_stock = int(
                        input(
                            "Quel produit voulez-vous réapprovisionner (ID) ? "
                            )
                    )
...
if len(inventory) == 0:
    print(
        "Aucun fichier Produits de sauvegarde trouvé. "
...
if len(list_user) == 0:
    print(
        "Aucun fichier Utilisateurs de sauvegarde trouvé. "
        "Chargement des données par défaut..."
    )
